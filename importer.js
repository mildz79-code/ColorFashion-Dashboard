...
async function importFile(filePath) {
  ...
  const { data: empRows } = await supabase
    .from('employees')
    .select('id, wasp_id')
    .in('wasp_id', employeeList.map(e => e.wasp_id));
  ...
  await supabase
    ...
    .update({
      ...
      status:          errorRecords.length === rows.length ? 'failed' : 
                       errorRecords.length > 0 ? 'partial' : 'completed',
    })
    .eq('id', runId);

  log(`Import complete — ${employeeList.length} employees, ${shiftsWithIds.length} shifts, $${totalGross.toFixed(2)} gross pay`);
...
// Graceful shutdown
process.on('SIGINT',  () => { log('Shutting down'); watcher.close(); process.exit(0); });
process.on('SIGTERM', () => { log('Shutting down'); watcher.close(); process.exit(0); });
